...
class TestCase:
    ...
    def setup_class(self):
        ...
        self.wait = WebDriverWait(self.dr.get_driver(), timeout=WAIT_TIMEOUT)
    ...
    def test_check_elements(self):
        ...
        assert self.wait.until(lambda _: len(self.dr.get_windows()) > 1), 'Не открылась новая вкладка'
        ...
        self.wait.until(lambda _: self.dr.get_windows()[0] == window_tensor), 'Не перешел на вкладку tensor.ru'
    ...
    def test_change_region(self):
        ...
        self.wait.until(lambda d: self.page_contacts.current_region.text != region_name)
        ...
        # Проверим отображение списков партнера по городу (Проверяем после выбора
        # потому что до него список не отображается, возможно ошибка ?)
        assert self.page_contacts.city_list.is_displayed() and self.page_contacts.partners_city_list.is_displayed(),\
